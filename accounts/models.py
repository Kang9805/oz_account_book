...
class Account(models.Model):
    ...
    account_number = models.CharField(
        max_length=20, unique=True, verbose_name="계좌 번호"
    )

    # 🌟 BANK_CHOICES 반영
    bank_code = models.CharField(
        max_length=3, choices=BANK_CHOICES, verbose_name="은행 코드"
    )
    ...
    balance = models.DecimalField(
        max_digits=18, decimal_places=2, default=0, verbose_name="현재 잔액"
    )
...
class Transaction(models.Model):
    ...
    transaction_amount = models.DecimalField(
        max_digits=18, decimal_places=2, verbose_name="거래 금액"
    )
    ...
    transaction_details = models.CharField(
        max_length=255, blank=True, verbose_name="거래 설명/인자 내역"
    )
